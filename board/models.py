...
class Board(models.Model):
    ...
    def move(self,id,direction):
        ...
        log = "Coordonnées de base : " + str(board.pos2)
        directionString = ''.join(direction)
        coordMove = board.get_moves(directionString)
        log += "CoordMove : " + str(coordMove)
        #if not direction in board.get_moves(board, id):
        ...
        if id == 1:
            log+= "Calcul coord : " + str(board.pos1[0]) + "+" +  str(coordMove[0])
            log+= "Calcul coord : " + str(board.pos1[1]) + "+" +  str(coordMove[1])

            board.pos1[0]+=coordMove[0]
            board.pos1[1]+=coordMove[1]
            new_pos = board.pos1

            log += "New pos : " + str(new_pos)
        else:
            board.pos2 = [board.pos2[0]+directions[direction][0] , board.pos2[1]+directions[direction][1]]
            board.grid[board.pos2[0]][board.pos2[1]]= id
            new_pos = board.pos2
            
        log += "Changement effectue."
        log+= "Vérification des captures"
        if board.check_capture(direction,new_pos):
            log += "Capture en cours..."
            board.capture(direction,new_pos)
